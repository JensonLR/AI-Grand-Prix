 ...
 * Pit procedure, tyre strategy, local yellow/VSC, physical Safety Car pace control and
 * red-flag stoppage/restart all change authoritative car state; none are HUD-only effects.
 */
export class ChampionshipRaceSimulation extends RaceSimulation {
  ...
  override step(inputs:Map<string,Control>,dt=FIXED_DT){
    ...
    for(const car of this.state.cars){
      ...
      if(this.state.flag==='VSC'){
        throttle=Math.min(throttle,.42);if(car.speed>38)brake=Math.max(brake,.24);
      }
      if(this.state.flag==='SAFETY_CAR'){
        ...
        if(ahead){
          ...
          if(gap<.010){throttle=Math.min(throttle,.08);if(car.speed>Math.max(12,ahead.speed-1))brake=Math.max(brake,.44);}
        }
      }
    ...
    if(this.state.flag==='RED'){
      for(const car of this.state.cars){if(car.status==='RUNNING'){car.speed=0;car.vx=0;car.vz=0;car.controls={...car.controls,throttle:0,brake:1};}}
    }
    ...
    if(latest&&latest.id!==this.seenIncident&&latest.type!=='FLAG'){
      this.seenIncident=latest.id;
      this.handleIncident(latest);
    }
    this.advanceRaceControl();
  ...
  private handleIncident(incident:RaceIncident){
    ...
    if(incident.severity==='HIGH'){
      this.recentHigh=this.recentHigh.filter(t=>this.state.time-t<14);this.recentHigh.push(this.state.time);
      if(this.recentHigh.length>=2||this.state.wetness>.82){this.enterRedFlag(incident);return;}
      this.state.flag='SAFETY_CAR';this.flagUntil=Math.max(this.flagUntil,this.state.time+18);this.pushIncident('FLAG',incident.cars,'HIGH',incident.lap);return;
    }
    if(incident.severity==='MEDIUM'){
      this.state.flag='VSC';this.flagUntil=Math.max(this.flagUntil,this.state.time+7);this.pushIncident('FLAG',incident.cars,'MEDIUM',incident.lap);return;
    }
    if(incident.type==='OFF_TRACK'){
      this.state.flag='YELLOW';this.flagUntil=Math.max(this.flagUntil,this.state.time+3.5);this.pushIncident('FLAG',incident.cars,'LOW',incident.lap);
    }
  }

  private enterRedFlag(incident:RaceIncident){
    this.redOrder=[...this.state.cars].filter(c=>c.status==='RUNNING').sort((a,b)=>a.position-b.position).map(c=>c.id);
    this.state.flag='RED';this.flagUntil=this.state.time+8;this.restartPending=true;this.pushIncident('FLAG',incident.cars,'HIGH',incident.lap);
  }

  private advanceRaceControl(){
    if(this.state.flag==='CHEQUERED'||this.state.time<this.flagUntil)return;
    if(this.state.flag==='RED'&&this.restartPending){
  ...
  private prepareRollingRestart(){
    ...
    live.forEach((car,i)=>{
      const score=Math.max(0,leaderScore-i*spacing),lap=Math.floor(score),progress=score-lap,p=trackPoint(progress,this.trackId),tan=trackTangent(progress,this.trackId);
  ...
  private pushIncident(type:RaceIncident['type'],cars:string[],severity:RaceIncident['severity'],lap:number){
    const id=`${this.state.tick}-${type}-${cars.join('-')}-${this.state.incidents.length}`;
