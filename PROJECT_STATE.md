# PROJECT STATE — AI GRAND PRIX / CONNECTOME WORLD CHAMPIONSHIP

Updated: 2026-09-17

Status: **full-BANC 2027 browser release validated and publishable**

## Locked product

- Product: **AI GRAND PRIX**
- Championship: **CONNECTOME WORLD CHAMPIONSHIP**
- Brand line: `22 BRAINS. 11 MACHINES. ONE GRID.`
- Secondary line: `BIOLOGY AT 300 KM/H.`
- Grid: 22 persistent digital fly-driver phenotypes, 11 original constructors, two drivers per constructor
- Development branch: `connectome-world-championship`
- Tested publication branch: `browser-preview`

## Crashed-chat release blockers — resolved

The final outstanding release work from the previous build session is complete:

1. **BANC integrity drift fixed** — the pinned official public v2 artifact contains **11,752,828** directed neuron pairs, replacing the stale 11,510,975 expectation.
2. **Complete browser graph generated and committed** — **188,508** mapped BANC rows and all **11,752,828** directed pairs are represented in the sparse runtime assets.
3. **Hard runtime-readiness gate active** — a neural race refuses to start unless the full graph identity and array contract validate.
4. **Production graph verification active** — CI checks the graph before product tests and again after the Vite production copy.
5. **Real browser release smoke active** — Chromium must render the production cars and report the full BANC runtime identity.
6. **Tested browser build republished** — successful CI publishes the production artifact to `browser-preview`.

## BANC runtime identity

- source: BANC v888 / v2 neuron-pair connectivity
- materialization: 888
- mapped rows / graph nodes: **188,508**
- directed neuron pairs: **11,752,828**
- runtime schema: `BANC-V888-FULL-GRAPH/1`
- processed runtime bytes: **75,418,182**
- execution: Web Worker
- topology: complete pinned CSR graph
- dynamics: AGP-modelled sparse LIF-like activity, not a claim of complete biological physiology

Control path:

`race world -> AGP sensory transduction -> BANC sensory/visual populations -> BANC graph propagation -> descending/motor populations -> constrained decoder -> vehicle controls -> authoritative physics`

All 22 drivers share the same graph topology but maintain independent seeded phenotype/calibration and live neural state.

## Product state

### Championship

- 24-round 2027 season, Bahrain to Abu Dhabi
- selectable 24-circuit Quick Race
- Drivers' and Constructors' standings
- persistent round progression
- full FP1/FP2/FP3 -> Q1/Q2/Q3 -> Grand Prix flow
- Sprint Qualifying / Sprint flow on designated Sprint rounds
- measured qualifying grids
- Quick Race / Human Test / neutral tests isolated from persistent championship development

### Racing

- deterministic 120 Hz fixed-step simulation
- tyres, wear, temperature, punctures and wet-weather grip
- energy, fuel, damage and collisions
- constructor engineering trade-offs inside a bounded regulation envelope
- sporting pit entry / limiter / 2.4 s service / tyre change / exit
- weather-aware strategy
- yellow and VSC handling
- Safety Car pace/bunching control
- red-flag stop and controlled rolling restart logic
- 22-car timing/classification and circuit map

### Presentation

- premium Three.js race scene
- AGP-27 / AGP-01 race-car asset path with procedural fallback
- original constructor marks and livery palettes
- selected-circuit world rendering
- broadcast, trackside, chase, cockpit/T-cam, aerial and neural camera modes
- wet-weather spray and race audio
- desktop/mobile responsive product shell
- touch Human Test controls
- installable web-app manifest

### Validation

A green release run verifies:

- full BANC runtime contract
- visible product shell contract
- race product contract
- browser boot contract
- lint
- TypeScript/typecheck
- automated tests
- 24 / 24 neural circuit drivability
- 22 / 22 neutral Super Licence certification
- public Bahrain replay generation
- production build
- production BANC graph copy
- Chromium render/full-BANC smoke
- browser-preview publication

## Scientific boundary

Do not claim 22 independently reconstructed biological flies. The championship uses 22 persistent **digital driver phenotypes** derived from one shared BANC graph architecture.

Do not claim biophysical completeness. BANC connectivity is source data; racing transduction, neural dynamics, phenotype perturbations, learning and the motor decoder are AI Grand Prix modelling assumptions.

## Remaining work

There is no remaining blocker from the crashed release session. Further work is optional product/presentation expansion rather than missing core functionality:

- richer animated pit crews and pit-lane choreography
- more elaborate automated television/incident-replay direction and podium ceremony
- continued bespoke constructor/livery art refinement
- continued profiling and mobile-load optimisation after the current vendor code-splitting pass

These items must not be presented as prerequisites for the validated BANC/racing architecture.
