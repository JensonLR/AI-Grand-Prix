...
async function enterArchive(page){
  ...
  await page.waitForSelector('.paddock-v2',{state:'attached',timeout:10000});
  const archive=page.getByRole('button',{name:/RACE ARCHIVE/i});
  await archive.waitFor({state:'attached',timeout:10000});
  await archive.click({noWaitAfter:true});
  // React swaps the home shell for the race shell synchronously after replay load.
  // Poll the DOM rather than using waitForSelector's visibility machinery, which
  // proved flaky under the high-DPR mobile Chromium job despite the node resolving visible.
...
try{
  ...
  const mobileSnapshot=await mobile.evaluate(()=>{
    ...
    return{
      width:innerWidth,height:innerHeight,car:document.documentElement.dataset.agpCar,visual:document.documentElement.dataset.agpVisual,
      race:document.querySelector('.screen-race .cwc-broadcast')!==null,
      canvasWidth:canvasRect?.width??0,canvasHeight:canvasRect?.height??0,
      backingWidth:canvas instanceof HTMLCanvasElement?canvas.width:0,backingHeight:canvas instanceof HTMLCanvasElement?canvas.height:0,
      ...
      miniDisplay:mini?getComputedStyle(mini).display:'absent',firstCameraHeight:firstCamera?.height??0,visibleDrivers
    };
  });
  ...
  if(await range.count()){
    ...
    const results=await mobile.evaluate(()=>{
      ...
      const dock=document.querySelector('.camera-bar');
      return{sheet,title,podium,list,actions,dockDisplay:dock?getComputedStyle(dock).display:'none'};
    });
