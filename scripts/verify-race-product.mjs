...
const rejectText=(source,text,label)=>{if(source.includes(text))throw new Error(`Race product contract failed: ${label}`)};
...
requireText(app,"edges=n?.graphEdges??11752828",'Connectome Lab fallback does not match the verified full graph');
rejectText(app,'11510975','stale BANC directed-pair identity remains in player-facing UI');
...
requireText(worker,'BANC_EDGES=11752828','worker full-graph edge identity is stale');
