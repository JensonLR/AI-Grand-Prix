...
if(!cls.includes('private currentTrackId=')){
  cls=cls.replace("private lowPower=matchMedia('(max-width: 760px), (prefers-reduced-motion: reduce)').matches;","private lowPower=matchMedia('(max-width: 760px), (prefers-reduced-motion: reduce)').matches;private currentTrackId=DEFAULT_2027_TRACK_ID;private trackGroup=new THREE.Group();");
}
// Existing circuit queries inside the class all go through the selected track.
cls=cls.replaceAll('trackPoint(','this.tp(').replaceAll('trackTangent(','this.tt(');
// Base-mode construction remains valid even when used outside the championship adapter.
...
// Circuit-created geometry belongs to one group so it can be replaced without touching cars/lights/clouds.
...
source=source.replace('function trackProgress(x:number,z:number){let best=0,dist=Infinity;for(let i=0;i<180;i++){const p=trackPoint(i/180),d=(p.x-x)**2+(p.z-z)**2;if(d<dist){dist=d;best=i/180;}}return best;}','function trackProgress(x:number,z:number,trackId=DEFAULT_2027_TRACK_ID){let best=0,dist=Infinity;for(let i=0;i<256;i++){const p=trackPoint(i/256,trackId),d=(p.x-x)**2+(p.z-z)**2;if(d<dist){dist=d;best=i/256;}}return best;}');
