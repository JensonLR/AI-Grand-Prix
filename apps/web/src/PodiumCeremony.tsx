...
import { driverById,teamFor } from './championship';
...
export function PodiumCeremony({cars}:{cars:CarState[]}){
  ...
  return <section className="podium-ceremony"><div className="podium-light"/><div className="podium-trophy"><Trophy/><small>CONNECTOME WORLD CHAMPIONSHIP</small><strong>GRAND PRIX WINNER</strong></div><div className="podium-grid">{slots.map((car,slot)=>{const driver=driverById(car.id),team=driver?teamFor(driver):undefined,place=slot===1?1:slot===0?2:3;return <article className={`podium-place p${place}`} style={{'--podium':car.colour,'--podium-accent':driver?.accent??car.colour} as React.CSSProperties} key={car.id}><header>{team&&<TeamMark teamId={team.id}/>}<span>P{place}</span></header><div className="podium-number">{car.number}</div><strong>{car.name}</strong><small>{team?.name??'AGP DEVELOPMENT'}</small><div className="podium-block"><b>{place}</b></div></article>})}</div><div className="podium-confetti" aria-hidden>{Array.from({length:24},(_,i)=><i key={i} style={{'--x':`${(i*37)%100}%`,'--delay':`${(i%7)*-.22}s`,'--spin':`${(i%5)*47}deg`} as React.CSSProperties}/>)}</div></section>
}
