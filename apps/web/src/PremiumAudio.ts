export class PremiumRaceAudio {
  ...
  dispose(){for(const o of this.engine){try{o.stop();}catch(error){void error;}}try{this.neural?.stop();}catch(error){void error;}try{this.wind?.stop();}catch(error){void error;}try{this.tyre?.stop();}catch(error){void error;}void this.ctx?.close();this.ctx=null;this.master=null;}
}
